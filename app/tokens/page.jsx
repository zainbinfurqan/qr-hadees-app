  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {loading && <p className="text-center text-gray-600">Loading tokens...</p>}
        ...
        {!loading && !error && tokens.length > 0 && (
          <div className="overflow-x-auto">
                {tokens.map((token, index) => (
                  <div key={index} className="hover:bg-gray-100 dark:hover:bg-gray-800">
                    <div className="border p-3 text-sm break-all">{token.fcmtoken}</div>
                    {/* <td className="border p-3 text-sm">{token.createdAt ? new Date(token.createdAt).toLocaleString() : 'N/A'}</td> */}
                  </div>
                ))}
